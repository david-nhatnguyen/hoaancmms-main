...
import { Plus, Trash2, Upload, Filter, Search, Link2, X, AlertCircle } from "lucide-react";
...
export function PMPlanStep3({
  ...
  const activeChecklists = (checklistTemplates as any[]).filter(
    (c) => c.status === ChecklistStatus.ACTIVE,
  );
  ...
  const availableEquipments = useMemo(() => {
    ...
    let filtered = factoryEquipments.filter(
      (e) => !addedIds.includes(e.id) && !companionIds.includes(e.id),
    );
    if (groupFilter) {
  ...
  const availableCompanionEquipments = useMemo(() => {
    ...
    return factoryEquipments.filter(
      (e) =>
        e.id !== mainItem.equipmentId && !addedIds.includes(e.id) && !companionIds.includes(e.id),
    );
  ...
  const updateItem = (
    itemId: string,
    field: keyof PMPlanEquipmentItem,
    ...
    <div className="space-y-4">
                ...
                >
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      ...
                      >
                        <Link2 className="h-3 w-3 mr-1" />+ Thêm TB đi kèm
                      </Button>
                    </div>
                  </TableCell>
                  <TableCell>
      ...
      {items.length > 0 && (
        <p className="text-sm text-muted-foreground">
          Tổng cộng:{" "}
          <span className="font-medium text-foreground">{items.length} thiết bị chính</span>
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="bg-card border-border max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Chọn thiết bị để thêm vào kế hoạch bảo dưỡng</DialogDescription>
          </DialogHeader>
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="pt-4 border-t border-border">
              <p className="text-xs text-muted-foreground mb-2">
                <strong>Lưu ý:</strong> Thiết bị đi kèm sẽ kế thừa ngày kế hoạch và người phụ trách
                từ thiết bị chính.
              </p>
              <Button className="w-full" onClick={() => setShowCompanionSheet(false)}>
                Xong
              </Button>
            </div>
          </div>
