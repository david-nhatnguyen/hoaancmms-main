      ...
      <CommandList>
        ...
        <CommandGroup>
          {factories.map((factory) => (
            ...
            >
              <Check
                className={cn("mr-2 h-4 w-4", value === factory.id ? "opacity-100" : "opacity-0")}
              />
              {factory.name}
            </CommandItem>
          ))}
  ...
  const triggerButton = (
    ...
    >
      <div className="flex items-center gap-2 truncate">
        ...
        <span className="truncate">{!isLoading && (selectedLabel || placeholder)}</span>
      </div>
    </Button>
  );
      ...
      <Drawer open={open} onOpenChange={setOpen}>
        ...
        <DrawerContent>
          <div className="mx-auto w-full max-w-sm">
            ...
            <div className="px-4 pb-4">{factoryListContent}</div>
          </div>
        </DrawerContent>
      </Drawer>
    );
