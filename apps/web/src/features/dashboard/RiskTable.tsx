...
export function RiskTable({ data, compact = false, className }: RiskTableProps) {
  ...
  const getRiskBadge = (level: RiskEquipment["riskLevel"]) => {
    ...
    return <span className={cn("status-badge text-xs", styles[level])}>{labels[level]}</span>;
  };
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground truncate">{eq.equipmentName}</p>
                </div>
              ...
              <div className="flex items-center gap-4 text-xs">
                ...
                <span
                  className={cn(
                    eq.incidents > 1 && "text-[hsl(var(--status-maintenance))] font-medium",
                  )}
                >
                  Sự cố: {eq.incidents}
                </span>
  ...
  return (
    <div
      className={cn(
        "bg-card rounded-xl border border-border/50 overflow-hidden flex flex-col",
        className,
      )}
    >
      {compact && (
      ...
      <Table>
        ...
        <TableBody>
          {displayData.map((eq) => (
            <TableRow
              key={eq.id}
              className={cn("table-row-interactive", eq.riskLevel === "high" && "bg-destructive/5")}
            >
              <TableCell>
                <div>
                  <p className={cn("font-medium", eq.riskLevel === "high" && "text-destructive")}>
                    {eq.equipmentCode}
                  </p>
              ...
              {!compact && (
                <TableCell className="text-sm text-muted-foreground">{eq.equipmentGroup}</TableCell>
              )}
              <TableCell className="text-center">
                <span className={cn("font-semibold", eq.pmOverdue > 0 && "text-destructive")}>
                  {eq.pmOverdue}
                </span>
              </TableCell>
              <TableCell className="text-center">
              ...
              <TableCell className="text-center">
                <span className={cn("font-semibold", eq.downtime > 4 && "text-destructive")}>
                  {eq.downtime}h
                </span>
              </TableCell>
              <TableCell className="text-center">{getRiskBadge(eq.riskLevel)}</TableCell>
              {!compact && (
