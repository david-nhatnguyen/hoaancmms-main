  ...
  return (
    ...
    >
        ...
        {data.trendValue && (
          <div
            className={cn(
              ...
              data.trend === "up" &&
                data.status !== "danger" &&
                "bg-status-active/10 text-[hsl(var(--status-active))]",
              data.trend === "down" &&
                data.status === "danger" &&
                "bg-status-active/10 text-[hsl(var(--status-active))]",
              data.trend === "down" &&
      ...
      <div className="space-y-0 md:space-y-1 min-w-0">
        <p className="text-[10px] md:text-sm text-muted-foreground truncate leading-tight">
          {data.label}
        </p>
        <div className="flex items-baseline gap-0.5">
          <span
            className={cn("text-lg md:text-3xl font-bold leading-none", valueStyles[data.status])}
          >
            {data.value}
          </span>
          {data.unit && (
