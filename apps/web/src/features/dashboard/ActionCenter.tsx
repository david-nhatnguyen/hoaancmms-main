...
import { Clock, AlertTriangle, AlertCircle, ChevronRight, ExternalLink } from "lucide-react";
...
const typeConfig = {
  ...
  "high-risk": {
    icon: AlertTriangle,
    label: "Rủi ro cao",
    color: "text-[hsl(var(--status-maintenance))]",
  },
...
const priorityStyles = {
  ...
  medium:
    "bg-status-maintenance/10 text-[hsl(var(--status-maintenance))] border-status-maintenance/20",
...
export function ActionCenter({ data, compact = false, className }: ActionCenterProps) {
  ...
  return (
    ...
    >
            ...
            >
              <div className="flex items-start gap-2 md:gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs md:text-sm font-medium mb-0.5 md:mb-1 truncate">
                  ...
                  <div className="flex flex-wrap gap-1.5 md:gap-2 mt-2 md:mt-3">
                    {action.actions
                      .slice(0, isMobile ? 1 : action.actions.length)
                      .map((act, idx) => (
                        <Button
                          key={idx}
      ...
      {data.length === 0 && (
        <div className={cn("text-center text-muted-foreground", isMobile ? "p-6" : "p-8")}>
          <AlertCircle
            className={cn("mx-auto mb-2 opacity-50", isMobile ? "h-6 w-6" : "h-8 w-8")}
          />
