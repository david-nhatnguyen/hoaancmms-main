...
export function InsightPanel({ title, insights, type = "info" }: InsightPanelProps) {
  ...
  return (
    <div
      className={cn("rounded-xl border p-2.5 md:p-4 max-w-full overflow-hidden", styles.container)}
    >
      ...
      <ul className="space-y-1 md:space-y-2">
        {insights.slice(0, 3).map((insight, idx) => (
          <li key={idx} className="flex items-start gap-1.5 text-[11px] md:text-sm">
            <span
              className={cn("h-1 w-1 md:h-1.5 md:w-1.5 rounded-full mt-1.5 shrink-0", styles.dot)}
            />
