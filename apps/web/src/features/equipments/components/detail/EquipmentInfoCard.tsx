...
import { Building2, Package, Calendar, Wrench, Cpu, Info } from "lucide-react";
...
export const EquipmentInfoCard = ({ equipment }: EquipmentInfoCardProps) => {
  ...
  const InfoRow = ({
    ...
    >
      ...
      <div className={cn("font-medium", isMobile ? "text-sm pl-6" : "w-2/3 text-sm")}>
        {value || "—"}
      </div>
    </div>
  );
    ...
    >
      ...
      <Card className="bg-card border-border/50 shadow-sm transition-all hover:shadow-md">
        ...
        <CardContent className={cn("pt-2", isMobile ? "p-3 pt-0" : "p-6 pt-2")}>
          <InfoRow label="Tên thiết bị" value={equipment.name} />
          <InfoRow
            label="Mã thiết bị"
            value={
          ...
          />
          <InfoRow label="Loại thiết bị" value={equipment.category} />
          <InfoRow
            label="Nhà máy"
            value={
              equipment.factory?.name || equipment.factoryName ? (
                <button
                  onClick={() =>
                    navigate(`/equipments?factory=${equipment.factory?.id || equipment.factoryId}`)
                  }
            ...
            icon={Building2}
          />
          <InfoRow label="Số lượng" value={equipment.quantity} icon={Package} />
        </CardContent>
      </Card>
      ...
      <Card className="bg-card border-border/50 shadow-sm transition-all hover:shadow-md">
        ...
        <CardContent className={cn("pt-2", isMobile ? "p-3 pt-0" : "p-6 pt-2")}>
          <InfoRow label="Thương hiệu" value={equipment.brand} />
          <InfoRow label="Xuất xứ" value={equipment.origin} />
          <InfoRow label="Năm sản xuất" value={equipment.modelYear} icon={Calendar} />
          <InfoRow label="Kích thước" value={equipment.dimension} />
        </CardContent>
      </Card>
