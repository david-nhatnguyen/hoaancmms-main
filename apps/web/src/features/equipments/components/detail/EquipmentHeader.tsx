...
import { Pencil, Layers, Maximize2, Image as ImageIcon, QrCode, X } from "lucide-react";
...
export const EquipmentHeader = ({ equipment }: { equipment: Equipment }) => {
  ...
  return (
    <>
      ...
      <QRPreviewDialog isOpen={isQROpen} onClose={() => setIsQROpen(false)} equipment={equipment} />

      {/* Image Preview Dialog */}
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent
          hideCloseButton
          className="max-w-4xl p-0 overflow-hidden bg-transparent border-none shadow-none flex items-center justify-center"
        >
            ...
            <div className="absolute top-4 right-4 z-50">
              <DialogClose asChild>
                <Button
                  variant="secondary"
                  size="icon"
                  className="rounded-full h-10 w-10 bg-black/50 text-white hover:bg-black/70 border-none backdrop-blur-sm"
                >
                  <X className="h-5 w-5" />
                </Button>
              </DialogClose>
