    ...
    <div className="bg-card rounded-xl border border-border/50 overflow-hidden">
      ...
      <div className="grid grid-cols-7 border-b border-border/50">
        {WEEKDAYS.map((day, idx) => (
          <div
            key={day}
            className={cn("py-3 text-center text-sm font-medium", idx === 0 && "text-destructive")}
          >
            {day}
          </div>
        ))}
              ...
              {day && (
                <>
                  <div className="flex items-center justify-between mb-2">
                    <span className={cn("text-sm font-medium", isSunday && "text-destructive")}>
                      {day}
                    </span>
              ...
              <div className="p-4 rounded-lg bg-muted/50 space-y-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {selectedWorkOrder.equipmentGroup}
                  </p>
                </div>
              ...
              {selectedWorkOrder.status === "in-progress" && (
                <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    Đã hoàn thành {selectedWorkOrder.items.filter((i) => i.result !== null).length}/
                    {selectedWorkOrder.items.length} hạng mục
                  </p>
                </div>
              )}
