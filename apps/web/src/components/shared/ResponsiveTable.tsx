      ...
      return (
        <div className="space-y-4 pt-1 pb-4">
          {[1, 2, 3].map((i) => (
            <div
              key={`mob-skel-${i}`}
              className="bg-card rounded-xl border border-border/50 p-5 space-y-4 animate-pulse"
            >
      ...
      <div className="space-y-0">
        ...
        <div className="space-y-4 max-w-full overflow-x-hidden pt-1 pb-4">
          {items.map((item) => {
            if (renderMobileCard) {
              return (
                <div key={keyExtractor(item)} className="max-w-full overflow-hidden">
                  {renderMobileCard(item, columns, selectedIdsSet.has(keyExtractor(item)), () =>
                    toggleSelectItem(keyExtractor(item)),
                  )}
                </div>
              );
            }
            ...
            const primaryCol =
              columns.find((c) => c.mobilePriority === "primary") ||
            ...
            const imageCol = columns.find(
              (c) => c.key === "image" || c.mobilePriority === ("image" as any),
            );
            const qrCol = columns.find(
              (c) => c.key === "qrCode" || c.mobilePriority === ("qr" as any),
            );
            const statusCol = columns.find(
              (c) => c.key === "status" || c.mobilePriority === ("status" as any),
            );
            ...
            return (
              <div key={keyExtractor(item)}>
                <MobileCard
                  ...
                  actionSlot={
                    qrCol ? (qrCol.mobileRender?.(item) ?? qrCol.render(item)) : undefined
                  }
            ...
            {isSelectable && (
              <TableHead className="w-[40px] px-4">
                <Checkbox
                  checked={
                    items.length > 0 &&
                    items.every((item) => selectedIdsSet.has(keyExtractor(item)))
                  }
            ...
            {columns.map((col) => (
              ...
              >
                {typeof col.header === "function"
                  ? col.mobileLabel || col.key
                  : col.header || col.mobileLabel || col.key}
              </TableHead>
            ))}
          </TableRow>
        </TableHeader>
            ...
            items.map((item) => (
              <TableRow
                ...
                className={cn(
                  ...
                  selectedIdsSet.has(keyExtractor(item)) &&
