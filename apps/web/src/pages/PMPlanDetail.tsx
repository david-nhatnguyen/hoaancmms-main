...
import { pmPlans, PM_STATUS_LABELS, PM_ITEM_STATUS_LABELS, PMPlan } from "@/data/pmPlanData";
        ...
        <div className="flex items-center gap-2">
          ...
          {plan.status !== "locked" && (
            <>
              <Button
                variant="outline"
                onClick={() => navigate(`/pm-plans/${plan.id}/edit`)}
                className="action-btn-secondary"
              >
              ...
              {plan.status === "active" && (
                <Button
                  onClick={handleLock}
                  className="action-btn-primary bg-status-maintenance hover:bg-status-maintenance/90"
                >
              ...
              <TableBody>
                {plan.items.map((item) => (
                  ...
                  >
                    ...
                    <TableCell className="text-center">{getItemStatusBadge(item.status)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-secondary/30 rounded-lg">
                  ...
                  <span className="font-bold text-primary">
                    {plan.items.filter((i) => i.plannedDate && i.checklistId).length}/
                    {plan.items.length}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-secondary/30 rounded-lg">
                  ...
                  <span className="font-bold text-[hsl(var(--status-active))]">
                    {plan.items.filter((i) => i.status === "work-order-created").length}/
                    {plan.items.length}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-secondary/30 rounded-lg">
