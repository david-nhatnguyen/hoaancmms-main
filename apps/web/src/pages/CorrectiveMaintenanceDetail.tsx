...
export default function CorrectiveMaintenanceDetail() {
  ...
  const [repairStartTime, setRepairStartTime] = useState<string | null>(
    cm?.repairStartTime || null,
  );
  ...
  if (!cm) {
    return (
      <div className={cn("text-center py-12", isMobile ? "px-4" : "p-6")}>
        <AlertTriangle className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
        <p className="text-muted-foreground mb-4">Không tìm thấy sự cố</p>
        <Button
          variant="outline"
          onClick={() => navigate("/corrective-maintenance")}
  ...
  const getStatusBadge = (status: CorrectiveMaintenance["status"]) => {
    ...
    return (
      <span className={cn("status-badge text-xs", styles[status])}>{CM_STATUS_LABELS[status]}</span>
    );
  };
  ...
  const InfoRow = ({ label, value }: { label: string; value: React.ReactNode }) => (
    ...
    >
      <span className={cn("text-muted-foreground", isMobile ? "text-[11px]" : "text-sm")}>
        {label}:
      </span>
  ...
  return (
    <div className="min-h-screen bg-background max-w-full overflow-x-hidden">
      ...
      >
        {/* Header row */}
        <div className={cn("flex items-start gap-3 mb-3", isMobile && "flex-wrap")}>
          {!isMobile && (
            <Button
              variant="ghost"
              size="icon"
          ...
          <div className={cn("flex items-center gap-2 shrink-0", isMobile && "w-full mt-2")}>
            {status === "new" && (
              <Button
        ...
        {status !== "new" && (
          <div
            className={cn("flex flex-wrap gap-x-4 gap-y-1", isMobile ? "text-[11px]" : "text-sm")}
          >
            <div className="flex items-center gap-1.5 text-muted-foreground">
              <Clock className={cn(isMobile ? "h-3 w-3" : "h-4 w-4")} />
              <span>
                Bắt đầu:{" "}
                {repairStartTime ? new Date(repairStartTime).toLocaleTimeString("vi-VN") : "-"}
              </span>
            </div>
            <div className="flex items-center gap-1.5 text-muted-foreground">
              <span>
      ...
      {/* Content */}
      <div className={cn("space-y-3", isMobile ? "px-4 py-3" : "p-4 space-y-4")}>
        ...
        <Collapsible open={incidentOpen} onOpenChange={setIncidentOpen}>
          <div className="bg-card rounded-xl border border-border/50 overflow-hidden">
            ...
            >
              <h3 className={cn("font-semibold flex items-center gap-2", isMobile && "text-sm")}>
                <AlertTriangle
              ...
              {incidentOpen ? (
                <ChevronUp
                  className={cn("text-muted-foreground", isMobile ? "h-4 w-4" : "h-5 w-5")}
                />
              ) : (
                <ChevronDown
                  className={cn("text-muted-foreground", isMobile ? "h-4 w-4" : "h-5 w-5")}
            ...
            <CollapsibleContent>
              ...
              >
                <div className={cn("grid gap-0", isMobile ? "grid-cols-1" : "grid-cols-2 gap-4")}>
                  ...
                  <InfoRow
                    label="Thời điểm báo hỏng"
                    value={new Date(cm.reportedAt).toLocaleString("vi-VN")}
                  />
                  <InfoRow label="Người báo hỏng" value={cm.reportedBy} />
                </div>
                <div className={cn("bg-secondary/30 rounded-lg", isMobile ? "p-2.5" : "p-3")}>
                  <span
                    className={cn(
            ...
            <CollapsibleContent>
              ...
              >
                ...
                <div className="space-y-1.5">
                  ...
                  <Button
                    variant="outline"
                    className={cn("w-full border-dashed", isMobile ? "h-14" : "h-20")}
                    disabled={status === "closed"}
                  >
        ...
        <Collapsible open={timeOpen} onOpenChange={setTimeOpen}>
          <div className="bg-card rounded-xl border border-border/50 overflow-hidden">
            ...
            >
              <h3 className={cn("font-semibold flex items-center gap-2", isMobile && "text-sm")}>
                <Clock
                  className={cn("text-muted-foreground", isMobile ? "h-3.5 w-3.5" : "h-4 w-4")}
                />
                Thời gian & Downtime
              </h3>
              {timeOpen ? (
            ...
            <CollapsibleContent>
              ...
              >
                <div className={cn("grid gap-2", isMobile ? "grid-cols-2" : "grid-cols-2 gap-4")}>
                  <div className={cn("bg-secondary/30 rounded-lg", isMobile ? "p-2.5" : "p-4")}>
                    <span
                      className={cn("text-muted-foreground", isMobile ? "text-[10px]" : "text-sm")}
                    >
                      Bắt đầu sửa
                    </span>
                  ...
                  <div className={cn("bg-primary/10 rounded-lg", isMobile ? "p-2.5" : "p-4")}>
                    <span
                      className={cn("text-muted-foreground", isMobile ? "text-[10px]" : "text-sm")}
                    >
                      Thời gian sửa
                  ...
                  <div className={cn("bg-destructive/10 rounded-lg", isMobile ? "p-2.5" : "p-4")}>
                    <span
                      className={cn("text-muted-foreground", isMobile ? "text-[10px]" : "text-sm")}
                    >
                      Downtime
                    ...
                    <p
                      className={cn(
                        "font-bold text-destructive mt-0.5",
                        isMobile ? "text-sm" : "text-lg",
                      )}
                    >
          ...
          >
            ...
            <div className={cn("grid gap-0", isMobile ? "grid-cols-1" : "grid-cols-2 gap-4")}>
              ...
              <InfoRow
                ...
                value={cm.equipmentCondition || "Hoạt động bình thường"}
              />
            </div>
          </div>
        )}
        ...
        >
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <CheckCircle
                className={cn("text-[hsl(var(--status-active))]", isMobile ? "h-4 w-4" : "h-5 w-5")}
              />
          ...
          <div className="space-y-3">
            {/* Summary */}
            <div className={cn("bg-secondary/30 rounded-lg", isMobile ? "p-2.5" : "p-3")}>
              <div className={cn("flex justify-between mb-2", isMobile ? "text-xs" : "text-sm")}>
              ...
              <div className={cn("flex justify-between", isMobile ? "text-xs" : "text-sm")}>
                ...
                <span className="font-medium text-destructive">
                  {calculateDowntime(cm.reportedAt)}
                </span>
              </div>
            </div>
