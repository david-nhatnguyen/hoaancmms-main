...
import { workOrders, WO_STATUS_LABELS, WorkOrder, calculateProgress } from "@/data/workOrderData";
...
export default function WorkOrderList() {
  ...
  const [viewMode, setViewMode] = useState<"list" | "tablet" | "calendar">(
    isMobile ? "tablet" : "list",
  );
  ...
  const getStatusBadge = (status: WorkOrder["status"]) => {
    ...
    return <span className={cn("status-badge", styles[status])}>{WO_STATUS_LABELS[status]}</span>;
  };
  ...
  const ViewModeToggle = () => (
    ...
    >
      ...
      <Button
        ...
        className={cn("h-8 px-3 rounded-md", viewMode === "calendar" && "bg-background shadow-sm")}
      >
      ...
      <Button
        ...
        className={cn("h-8 px-3 rounded-md", viewMode === "tablet" && "bg-background shadow-sm")}
      >
  ...
  return (
    <div className={cn("animate-fade-in", isMobile ? "p-4" : "p-6")}>
      ...
      {/* Stats Cards */}
      <MobileStatsGrid stats={statsData} columns={{ mobile: 3, tablet: 3, desktop: 4 }} />

      {/* Search + Filters - Hide for calendar view */}
      {viewMode !== "calendar" && (
        <div className="space-y-4 mb-4">
          <div className="flex items-center gap-3">
            <div className={cn("relative flex-1", !isMobile && "max-w-sm")}>
          ...
          {!isMobile && <WorkOrderFilters filters={filters} onFiltersChange={setFilters} />}
        </div>
      )}
      ...
      {/* Calendar View */}
      {viewMode === "calendar" && <WorkOrderCalendar workOrders={workOrders} />}
            ...
            >
              ...
              <div className="mt-4 flex gap-2">
                ...
                {wo.status === "in-progress" && (
                  <Button
                    size="sm"
                    className="flex-1 bg-[hsl(var(--status-maintenance))] hover:bg-[hsl(var(--status-maintenance))]/90 text-white"
                  >
                    <Play className="h-4 w-4" />
                    Tiếp tục
                  </Button>
                ...
                {filteredOrders.length === 0 ? (
                  ...
                  filteredOrders.map((wo) => (
                    ...
                    >
                      ...
                      <TableCell className="text-sm">{wo.checklistName}</TableCell>
                      <TableCell className="text-center">
                        <span className="px-2 py-1 rounded-md bg-secondary text-xs font-medium">
                          {new Date(wo.plannedDate).toLocaleDateString("vi-VN")}
                        </span>
                      </TableCell>
                      <TableCell className="text-sm">{wo.assignee}</TableCell>
                      <TableCell className="text-center">
                        {wo.status !== "new" ? (
