...
import { Plus, Pencil, Trash2, Layers, Loader2, AlertTriangle } from "lucide-react";
...
export default function PermissionModuleList() {
  ...
  const handleSubmit = async (data: ModuleFormData) => {
    ...
    if (editingModule) {
      await updateModule.mutateAsync(
        {
          id: editingModule.id,
          payload: { name: data.name, description: data.description || undefined, sortOrder },
        },
  ...
  const handleBulkDelete = () => {
    ...
    setDeleteTarget({
      id: "bulk",
      name: `${selectedIds.length} module đã chọn`,
      description: "",
      sortOrder: 0,
    });
  };
    ...
    <PageContainer>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div className="stat-card flex items-center justify-between">
          <div>
            <p className="text-sm text-muted-foreground mb-1">Tổng modules</p>
        ...
        <div className="stat-card flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-bold text-primary">
              {isLoading
                ? "—"
                : modules && modules.filter((m) => (m._count?.permissions ?? 0) > 0).length}
            </p>
          </div>
          <div className="stat-card-icon bg-primary/20">
            <Layers className="h-5 w-5 text-primary" />
      ...
      {!isLoading && modules.length > 0 && (
        <div className="mt-4 flex items-start gap-2 text-xs text-muted-foreground">
          ...
          <span>
            Module đang được dùng bởi ít nhất một vai trò không thể xóa. Gỡ quyền khỏi tất cả vai
            trò trước khi xóa module.
          </span>
        </div>
      )}
      ...
      {/* ── Delete Confirmation ── */}
      <AlertDialog
        open={Boolean(deleteTarget) || isBulkDeleting}
        onOpenChange={(v) => {
          if (!v) {
            setDeleteTarget(null);
            setIsBulkDeleting(false);
      ...
      >
        <AlertDialogContent className="bg-card border-border">
          ...
          <AlertDialogFooter>
            ...
            >
              {deleteModule.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
              Xóa module
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
