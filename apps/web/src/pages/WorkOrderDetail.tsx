...
export default function WorkOrderDetail() {
  ...
  const getResultBadge = (result: "ok" | "ng" | "na" | null) => {
    ...
    return (
      <span className={cn("status-badge text-xs", styles[result])}>{RESULT_LABELS[result]}</span>
    );
  };
  ...
  return (
    ...
    >
      ...
      <Tabs defaultValue="checklist" className={cn(isMobile ? "space-y-3" : "space-y-6")}>
        <TabsList
          className={cn("bg-secondary/50 p-1", isMobile ? "w-full grid grid-cols-3 h-auto" : "")}
        >
          <TabsTrigger
            value="checklist"
            className={cn(
        ...
        <TabsContent value="checklist">
          ...
          <div
            className={cn("grid gap-2 mb-4", isMobile ? "grid-cols-4" : "grid-cols-4 gap-4 mb-6")}
          >
            <div
              className={cn(
            ...
            >
              <p className={cn("font-bold text-destructive", isMobile ? "text-xl" : "text-3xl")}>
                {ngCount}
              </p>
              <p className={cn("text-muted-foreground", isMobile ? "text-[10px]" : "text-sm")}>
                NG
            ...
            >
              <p
                className={cn("font-bold text-muted-foreground", isMobile ? "text-xl" : "text-3xl")}
              >
                {naCount}
              </p>
              <p className={cn("text-muted-foreground", isMobile ? "text-[10px]" : "text-sm")}>
          ...
          <div className="bg-card rounded-xl border border-border/50 overflow-hidden">
            <div className={cn("border-b border-border/50", isMobile ? "p-3" : "p-4")}>
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-0.5">
                        <p className={cn("font-medium leading-tight", isMobile && "text-sm")}>
                          {item.task}
                        </p>
        ...
        <TabsContent value="info">
          ...
          >
            <div
              className={cn("bg-card rounded-xl border border-border/50", isMobile ? "p-3" : "p-6")}
            >
              <h3 className={cn("font-semibold mb-3", isMobile ? "text-sm" : "text-lg")}>
                Thông tin thiết bị
              </h3>
            ...
            >
              ...
              <div className="space-y-0">
                <InfoRow
                  icon={Calendar}
                  label="Ngày kế hoạch"
                  value={new Date(wo.plannedDate).toLocaleDateString("vi-VN")}
                />
