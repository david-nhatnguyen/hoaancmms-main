...
export class FactoryListPage {
  ...
  constructor(page: Page) {
    ...
    this.factoryRows = page
      .locator("tbody tr")
      .or(page.locator('div[class*="transition-all"][class*="bg-card"]'));
  ...
  async mockApi() {
    ...
    await this.page.route("**/api/factories?*", async (route) => {
      ...
      const responseData = {
        ...this.currentMockData,
        data: filteredData,
        meta: { ...this.currentMockData.meta, total: filteredData.length },
      };
      ...
      if (method === "PUT" || method === "PATCH") {
        ...
        if (index !== -1) {
          ...
          await route.fulfill({
            status: 200,
            contentType: "application/json",
            body: JSON.stringify(this.currentMockData.data[index]),
          });
        } else {
